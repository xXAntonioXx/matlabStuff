function [clusters]=CrearCentroides(data,k)
    ...
    minimo=min(dataArray);%esto nos devuelve un vector con los valores minimos de cada columna
    maximo=max(dataArray);%valores maximos de cada columna
    
    clusters=[k,dimensiones(2)+1];%inicializamos la tabla de clusters
    
    for v=1:k%este for recorre renglon por renglon
        for c=1:dimensiones(2)-1
